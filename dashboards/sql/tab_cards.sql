...
SELECT
    ...
    CASE
        ...
        -- Google Business Profile / Google My Business / Google-organic clicks
        -- (utm_source=google with utm_medium=gbp|gmb|organic, utm_campaign=gbp_profile|
        -- gmb_consultation) are ORGANIC Google listing/maps clicks — classify as Organic
        -- Search, never Direct/Referral. Placed AFTER the paid detections (so a genuine
        -- paid lead still wins) and BEFORE the event_source / attribution Direct/Referral
        -- branches. e.g. Muhammad Fazal (gbp_profile → else 'Direct traffic') and Dinesh
        -- Sivasubramanium (gmb_consultation → else 'Referral').
    ...
    -- Google organic flag (GBP/GMB/organic — same signal as the refined_source branch)
    -- so the outer 'Organic/Direct with no pipeline -> No Activity' downgrade never
    -- demotes a genuine Google-organic lead (e.g. one who booked but has no opportunity).
